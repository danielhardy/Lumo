...
`EditDocument.currentVersion` remains 1. `RenderPipeline.cacheVersion` is 5: v2 introduced the
explicit Light stage, v3 recorded the refined non-neutral mapping, v4 clamps the curve's interior
points to stay monotonic, and v5 adds the GPU-backed Whites and Blacks endpoint stages (see below).
...
`CIToneCurve`, followed by Blacks as a separate low-end `CIToneCurve`; each changes its endpoint
and nearest quarter-tone with a smooth rolloff while leaving the opposite half of the range
effectively unchanged. Strong positive Whites and negative Blacks may produce out-of-range values
in the lazy graph, which are finite and clip only when rasterized. Neutral Light produces no nodes
and is an exact render identity.
