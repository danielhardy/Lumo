...
final class PackageSettingsTests: XCTestCase {
    ...
    private static var manifest: String {
        get throws {
            let url = URL(fileURLWithPath: #filePath)   // Tests/LUTzyKitTests/PackageSettingsTests.swift
    ...
    /// Every target, not just the library. `LUTzyKit` was clean before this step; the executable and
    ...
    func testEveryTargetIsInSwift6LanguageMode() throws {
        ...
        XCTAssertEqual(
            declared, 3,
            """
            All three targets — LUTzyKit, LUTzy, LUTzyKitTests — must declare Swift 6 language mode. \
        ...
        for target in ["LUTzyKit", "LUTzy", "LUTzyKitTests"] {
